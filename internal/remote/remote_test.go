...
func TestParseFromValueHTTPS(t *testing.T) {
	repoURL, filePath, err := remote.ParseFromValue("https://github.com/org/repo.git//src/main.py")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if filePath != "src/main.py" {
		t.Errorf("filePath = %q, want %q", filePath, "src/main.py")
	}
}
...
func TestCacheDirExtractsRepoName(t *testing.T) {
	tests := []struct {
		...
		{"git@github.com:phasecurve/readme-merge.git", "readme-merge"},
...
func TestResolverReadFile(t *testing.T) {
	...
	content, err := resolver.ReadFile(
		"git@github.com:phasecurve/readme-merge.git//README.md",
		"main",
	)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if !strings.Contains(content, "readme-merge") {
		t.Errorf("content should mention readme-merge, got:\n%.200s", content)
	}
}

func TestResolverReadFileWithBranchRef(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping network test in short mode")
	}
	...
	if !strings.Contains(content, "readme-merge") {
		t.Error("expected content to reference readme-merge")
	}
}
...
func TestResolverReusesCachedClone(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("second read (cached) failed: %v", err)
	}
}
...
func TestResolverFileNotFound(t *testing.T) {
	...
	_, err := resolver.ReadFile(
		"git@github.com:phasecurve/readme-merge.git//nonexistent-file.txt",
		"main",
	)
	if err == nil {
